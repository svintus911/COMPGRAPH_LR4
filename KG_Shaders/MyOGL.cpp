...
void OpenGL::render()
{
	...
	glLoadIdentity();
	gluPerspective(60, aspect, .1, 1000.0);
	...
	for (auto it = renderFunc.begin(); it != renderFunc.end(); it++)
	{
		(*it)(this);
	}
	...
	wchar_t *text = new wchar_t[message.size() ];
	mbstowcs(text, message.c_str(), message.size());
	//рисуем текст
	DrawText(g_hDC, text, -1, &r, 0);
	delete text;
	*/
...
bool OpenGL::isKeyPressed(int key)
{
	{
...
bool OpenGL::isKeyPressed2(int key)
{
	bool rrr = GetFocus() == g_hWnd;
	return (isKeyPressed(key) && (GetFocus() == g_hWnd));
}
